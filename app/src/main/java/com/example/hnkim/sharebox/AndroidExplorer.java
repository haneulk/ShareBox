...
public class AndroidExplorer {
    public static final int REQUEST_PERMISSION_READ_EXTERNAL_STORAGE = 10511;
    public static final int REQUEST_PERMISSION_WRITE_EXTERNAL_STORAGE = 10512;

    public static ArrayList<File> getFileListLike(File dir, String likeStr, ArrayList<File> list) {
        if(list==null) list = new ArrayList<>();
        for(File file : dir.listFiles()) {
            if(file.isDirectory())
                list = getFileListLike(file, likeStr, list);
            else {
                if(likeStr==null || file.getName().contains(likeStr))
                    list.add(file);
            }
        }
        return list;
    }
    public static long getDirectorySizeLike(File file, String likeStr) {
        long ret = 0;
        for(File cf : file.listFiles()) {
            if(cf.isDirectory())
                ret += getDirectorySizeLike(cf, likeStr);
            else {
                if(likeStr==null)
                    ret += cf.length();
                else if(file.getName().indexOf(likeStr) > 0)
                    ret += cf.length();
            }
        }
        return ret;
    }
    public static long getDirectorySize(File file, String[] fileExtends) {
        long ret = 0;
        for(File cf : file.listFiles()) {
            if(cf.isDirectory())
                ret += getDirectorySize(cf, fileExtends);
            else {
                if(fileExtends==null)
                    ret += cf.length();
                else {
                    for (String ex : fileExtends) {
                        ...
                        if(fileName.length() < ex.length())
                            continue;
                        String strex = fileName.substring(fileName.length() - ex.length(), fileName.length() - 1);
                        if (strex.equals(ex)) {
                            ret += cf.length();
                            break;
                        }
    ...
    // return directory's file count(include subdirectory)
    // count file and directory
    public static int getDirectoryCount(File dir, String[] fileExtends) {
        int ret = 0;
        for(File cf : dir.listFiles()) {
            if(cf.isDirectory()) {
                if(fileExtends==null)
                    ++ret;
                ret += getDirectoryCount(cf, fileExtends);
            }
            else {
                if(fileExtends==null)
                    ++ret;
                    ...
                    for (String ex : fileExtends) {
                        ...
                        if(fileName.length() < ex.length())
                            continue;
                        String strex = fileName.substring(fileName.length() - ex.length(), fileName.length());
                        if (strex.equals(ex)) {
                            ++ret;
                            break;
                        }
    ...
    // except input dir
    public static int getDirectoryCountOnlyDir(File dir) {
        int ret = 0;
        for(File cf : dir.listFiles()) {
            if(cf.isDirectory()) {
                ++ret;
                ret += getDirectoryCountOnlyDir(cf);
            }
        }
        return ret;
    }
    ...
    // except input dir
    public static int getDirectoryCountOnlyFile(File dir, String[] fileExtends) {
        int ret = 0;
        for(File cf : dir.listFiles()) {
            if(cf.isDirectory())
                ret += getDirectoryCountOnlyFile(cf, fileExtends);
            else {
                if(fileExtends==null)
                    ++ret;
                else {
    ...
    public static final HashMap<String, String> mimeMap = new HashMap<String, String>() {{put(".3dm","x-world/x-3dmf");
        ...
        put(".hwp","application/*");
        put(".pptx","application/powerpoint");
        put(".xlsx","application/excel");
    }};
    public static String getMime(String fileName) {
        Log.d("TAG", ".apk: " + mimeMap.get(".apk"));
        String m = mimeMap.get(fileName.substring(fileName.lastIndexOf('.')).toLowerCase());
        return m; //.substring(0, m.indexOf('/')) + "/*";
    }

    private static final String[] LIST_CAT_IMAGE = {"png", "jpg", "jpeg", "bmp"};    //이미지 파일 확장자
    private static final String[] LIST_CAT_MUSIC = {"mp3", "wav", "wma, m4a", "ogg"};             //음악 파일 확장자(mp3파일 + 음성녹음, 통화녹음파일-m4a)
    private static final String[] LIST_CAT_VIDEO = {"avi", "flv", "wmv", "mp4", "3gp"};     //비디오 파일 확장자
    private static final String[] LIST_CAT_DOCU = {"txt", "hwp", "ppt", "pdf", "doc", "pptx", "xlsx"};     //문서 파일 확장자
    private static final String[] LIST_CAT_APP = {"apk"};                               //앱 파일 확장자
    public static final int CAT_IMAGE = 10312;
    public static final int CAT_MUSIC = 10313;
    public static final int CAT_VIDEO = 10314;
    public static final int CAT_DOCU = 10315;
    public static final int CAT_APP = 10316;
    static public int getFileCategory(String fileName) {
        String ex = fileName.substring(fileName.lastIndexOf('.')+1);
        if(ex==null || ex.length()==0) return 0;
        for(String str : LIST_CAT_IMAGE)
            if(ex.equals(str)) return CAT_IMAGE;
        for(String str : LIST_CAT_MUSIC)
            if(ex.equals(str)) return CAT_MUSIC;
        for(String str : LIST_CAT_VIDEO)
            if(ex.equals(str)) return CAT_VIDEO;
        for(String str : LIST_CAT_DOCU)
            if(ex.equals(str)) return CAT_DOCU;
        for(String str : LIST_CAT_APP)
            if(ex.equals(str)) return CAT_APP;
        return 0;
    }
}
